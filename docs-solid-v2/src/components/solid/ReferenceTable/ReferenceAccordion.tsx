...
import { visuallyHidden } from '@msviderok/base-ui-solid/utils';
...
export default function ReferenceAccordion(props: Props) {
  ...
  return (
    <Accordion.Root aria-describedby={captionId} class={props.class}>
      <span id={captionId} style={visuallyHidden as JSX.CSSProperties} aria-hidden="true">
      ...
      <For each={entries()}>
        {(entry, index) => {
          const displayType = replaceComponentPrefix(
            entry[1].type,
            props.renameFrom,
            props.renameTo,
          );
          const detailedDisplayType = replaceComponentPrefix(
            entry[1].detailedType ?? entry[1].type,
            props.renameFrom,
            props.renameTo,
          );
          ...
          const { type: shortPropTypeName, detailedType } = getShortPropType(entry[0], displayType);
          const ShortPropType = createMdxComponent(`\`${shortPropTypeName}\``, {
            rehypePlugins: rehypeSyntaxHighlighting,
            useMDXComponents: () => ({
              code: (codeProps: TableCodeProps) => (
                <TableCode {...codeProps} printWidth={entry[0] === 'children' ? 999 : undefined} />
              ),
            }),
          });

          const PropDefault = createMdxComponent(`\`${entry[1].default}\``, {
            rehypePlugins: rehypeSyntaxHighlighting,
            useMDXComponents: () => ({ code: TableCode }),
          });

          const PropDescription = entry[1].description
            ? createMdxComponent(entry[1].description, {
                rehypePlugins: rehypeSyntaxHighlighting,
                useMDXComponents: () => ({
          ...
          const ExampleSnippet = entry[1].example
            ? createMdxComponent(entry[1].example, {
                rehypePlugins: rehypeSyntaxHighlighting,
                useMDXComponents: () => ({
                  code: (codeProps: Record<string, unknown>) => (
                    <code class="Code data-inline:mx-[0.1em]" {...codeProps} />
          ...
          const id = `${props.name}-${entry[0]}`;
          const showExpandedType = Boolean(entry[1].detailedType) || detailedType;
          const DefaultValue =
            entry[1].required || entry[1].default === undefined ? null : PropDefault;
          ...
          return (
            <Accordion.Item>
              <Accordion.Trigger
                id={id}
                index={index()}
                aria-label={`${props.nameLabel ?? 'Prop'}: ${entry[0]},${entry[1].required ? ' required,' : ''} type: ${shortPropTypeName}${entry[1].default !== undefined ? ` (default: ${entry[1].default})` : ''}`}
                    ...
                    {showExpandedType ? (
                      <ReferenceTableTooltip.Root disableHoverablePopup>
                        ...
                        <ReferenceTableTooltip.Popup>
                          {entry[1].detailedType ? <PropDetailedType /> : <PropType />}
                        </ReferenceTableTooltip.Popup>
                    ...
                    <DescriptionList.Item>
                      ...
                      <DescriptionList.Details>
                        <a href={`#${id}`} class="Link">
                          <TableCode class="text-(--color-blue)">{props.name}</TableCode>
                        </a>
                      </DescriptionList.Details>
                    </DescriptionList.Item>
                    {PropDescription && (

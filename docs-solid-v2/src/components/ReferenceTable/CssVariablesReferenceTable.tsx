...
export function CssVariablesReferenceTable(props: Props) {
  return (
    <>
      <Accordion.Root class={clsx(props.class, 'xs:hidden')}>
        ...
        {Object.entries(props.data).map(([name, cssVariable], index) => {
          const CssVariableDescription = createMdxComponent(
      ...
      <Table.Root class={clsx('hidden xs:block', props.class)}>
        ...
        <Table.Body>
          {Object.entries(props.data).map(([name, cssVariable]) => {
            const CssVariableDescription = createMdxComponent(
              cssVariable.description ?? '',
              CREATE_MDX_OPTIONS,
            );

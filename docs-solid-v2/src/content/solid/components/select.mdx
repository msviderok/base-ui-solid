...
- **Form controls must have an accessible name**: It can be created using the `Field` component. See [Labeling a select](#labeling-a-select) and the [forms guide](/solid/handbook/forms).
...
Use the [Field](/solid/components/field) component to provide a visible label for the select trigger:
...
<Demo path="select/multiple" compact />
...
<Demo path="select/object-values" compact />

...
- **Form controls must have an accessible name**: It can be created using a `<label>` element or the `Field` component. See [Labeling a combobox](#input-inside-popup) and the [forms guide](/solid/handbook/forms).
...
<Demo path="combobox/multiple" compact />

### Input inside popup

`<Combobox.Input>` can be rendered inside `<Combobox.Popup>` to create a searchable select popup.

<Demo path="combobox/input-inside-popup" compact />

Use the [Field](/solid/components/field) component to provide a visible label for the combobox trigger:
...
<Demo path="combobox/grouped" compact />
...
<Demo path="combobox/async-single" compact />

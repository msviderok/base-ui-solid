...
<Demo path="autocomplete/hero" />

## Usage guidelines

- **Avoid when selection state is needed**: Use [Combobox](/solid/components/combobox) instead of Autocomplete if the selection should be remembered and the input value cannot be custom. Unlike Combobox, Autocomplete's input can contain free-form text, as its suggestions only _optionally_ autocomplete the text.
...
<Demo path="autocomplete/async" compact />
...
<Demo path="autocomplete/inline" compact />

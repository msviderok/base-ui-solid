...
<Demo path="utils/use-render/render" />
...
<Demo path="utils/use-render/render-callback" />
...
Radix UI uses an `asChild` prop, while Base UI uses a `render` prop. Learn more about how composition works in Base UI in the [composition guide](/solid/handbook/composition).
...
<PropsReferenceTable
  data={{
    ...
    props: {
      ...
      description:
        'Props to be spread on the rendered element.\n\nThey are merged with the internal props\n of the component, so that event handlers are merged,\n`class` strings and `style` properties are joined, and other external props overwrite the internal ones.',
    },
    ref: {

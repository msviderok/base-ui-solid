...
<Demo path="handbook/forms/hero" showExtraPlaygroundLink />
...
When using [Server Functions with Form Actions](https://react.dev/reference/rsc/server-functions#server-functions-with-use-action-state) you can return server-side errors from `useActionState` to the `errors` prop. A demo is available [here](/solid/components/form#submit-with-a-server-function).
...
<Demo path="handbook/forms/react-hook-form" showExtraPlaygroundLink />
...
<Demo path="handbook/forms/tanstack-form" showExtraPlaygroundLink />

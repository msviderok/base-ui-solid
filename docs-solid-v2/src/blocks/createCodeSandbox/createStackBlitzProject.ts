...
export function createStackBlitzProject(options: createStackBlitzProject.Options) {
  ...
  addHiddenInput(form, 'project[template]', 'create-solid-app');
...
function createRequestPayload(options: createStackBlitzProject.Options) {
  ...
  const files: Record<string, string> = {
    ...demoFilesToInclude,
    [`src/index${indexExtension}`]: customIndexFile ?? CSA.getRootIndex(demoLanguage === 'ts'),
    'public/index.html': CSA.getHtml({
      title,
  ...
  if (demoLanguage === 'ts') {
    files['tsconfig.json'] = CSA.getTsconfig();
  }

...
function CopyButton() {
  ...
  return (
    ...
    >
      <Tooltip.Trigger
        ref={buttonRef}
        class="box-border flex h-10 w-10 items-center justify-center rounded-md border border-gray-200 bg-gray-50 text-gray-900 outline-0 select-none hover:bg-gray-100 focus-visible:outline focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-blue-800 active:bg-gray-100"
        onClick={handleCopy}
        aria-label="Copy to clipboard"
        render={<Button disabled={copied()} focusableWhenDisabled />}
      >
        {copied() ? <CheckIcon class="h-5 w-5" /> : <ClipboardIcon class="h-5 w-5" />}
      </Tooltip.Trigger>
      <Tooltip.Portal>
        <Tooltip.Positioner sideOffset={8}>
          <Tooltip.Popup class="flex origin-(--transform-origin) flex-col rounded-md bg-[canvas] px-2 py-1 text-sm shadow-lg shadow-gray-200 outline-1 outline-gray-200 transition-[transform,scale,opacity] data-ending-style:scale-90 data-ending-style:opacity-0 data-instant:duration-0 data-starting-style:scale-90 data-starting-style:opacity-0 dark:shadow-none dark:-outline-offset-1 dark:outline-gray-300">
            <Tooltip.Arrow class="data-[side=bottom]:-top-2 data-[side=left]:right-[-13px] data-[side=left]:rotate-90 data-[side=right]:left-[-13px] data-[side=right]:-rotate-90 data-[side=top]:-bottom-2 data-[side=top]:rotate-180">
              <ArrowSvg />
            </Tooltip.Arrow>
            Copy
          </Tooltip.Popup>
      ...
      <Toast.Viewport class="outline-0">
        {toasts.map((toast) => (
          ...
          >
            ...
            >
              <Toast.Arrow class="data-[side=bottom]:-top-2 data-[side=left]:right-[-13px] data-[side=left]:rotate-90 data-[side=right]:left-[-13px] data-[side=right]:-rotate-90 data-[side=top]:-bottom-2 data-[side=top]:rotate-180">
                <ArrowSvg />
              </Toast.Arrow>
              <Toast.Content>
                <Toast.Description />
...
function StackedToasts() {
  ...
  return (
    <Toast.Portal>
      <Toast.Viewport class="fixed z-10 top-auto right-[1rem] bottom-[1rem] mx-auto flex w-[250px] sm:right-[2rem] sm:bottom-[2rem] sm:w-[300px]">
        {toasts.map((toast) => (
          <Toast.Root
            key={toast.id}
            toast={toast}
          ...
          >
            <Toast.Content class="overflow-hidden transition-opacity [transition-duration:250ms] data-[behind]:pointer-events-none data-[behind]:opacity-0 data-[expanded]:pointer-events-auto data-[expanded]:opacity-100">
              <Toast.Title class="text-[0.975rem] leading-5 font-medium" />
              <Toast.Description class="text-[0.925rem] leading-5 text-gray-700" />
              <Toast.Close
                class="absolute top-2 right-2 flex h-5 w-5 items-center justify-center rounded border-none bg-transparent text-gray-500 hover:bg-gray-100 hover:text-gray-700"
                aria-label="Close"
              >
                <XIcon class="h-4 w-4" />
              </Toast.Close>

...
export default function ExampleNavigationMenu() {
  return (
    <NavigationMenu.Root class="min-w-max rounded-lg bg-gray-50 p-1 text-gray-900">
      <NavigationMenu.List class="relative flex">
        <NavigationMenu.Item>
          <NavigationMenu.Trigger class={triggerclass}>
            Overview
            <NavigationMenu.Icon class="transition-transform duration-200 ease-in-out data-[popup-open]:rotate-180">
              <ChevronDownIcon />
            </NavigationMenu.Icon>
          ...
          <NavigationMenu.Content class={contentclass}>
            <ul class="grid list-none grid-cols-1 gap-0 sm:grid-cols-[12rem_12rem]">
              <For each={overviewLinks}>
                {(item) => (
                  <li>
                    <Link href={item.href} class={linkCardclass}>
                      <h3 class="m-0 mb-1 text-base leading-5 font-medium">{item.title}</h3>
                      <p class="m-0 text-sm leading-5 text-gray-500">{item.description}</p>
                    </Link>
                  </li>
                )}
              </For>
              <li>
                <NavigationMenu.Root orientation="vertical">
                  <NavigationMenu.Item>
                    <NavigationMenu.Trigger class={linkCardclass}>
                      <span class="m-0 mb-1 text-base leading-5 font-medium">Handbook</span>
                      <p class="m-0 text-sm leading-5 text-gray-500">
                        How to use Base UI effectively.
                      </p>
                      <NavigationMenu.Icon class="absolute top-1/2 right-2.5 flex h-2.5 w-2.5 -translate-y-1/2 items-center justify-center transition-transform duration-200 ease-in-out data-[popup-open]:rotate-180">
                        <ChevronRightIcon />
                      </NavigationMenu.Icon>
                    </NavigationMenu.Trigger>
                    <NavigationMenu.Content class={contentclass}>
                      <ul class="flex max-w-[400px] flex-col justify-center">
                        <For each={handbookLinks}>
                          {(item) => (
                            <li>
                              <Link href={item.href} class={linkCardclass}>
                                <h3 class="m-0 mb-1 text-base leading-5 font-medium">
                                  {item.title}
                                </h3>
                                <p class="m-0 text-sm leading-5 text-gray-500">
                                  {item.description}
      ...
      <NavigationMenu.Portal>
        ...
        >
          <NavigationMenu.Popup class="data-[ending-style]:easing-[ease] relative h-[var(--popup-height)] w-max origin-[var(--transform-origin)] rounded-lg bg-[canvas] text-gray-900 shadow-lg shadow-gray-200 outline outline-1 outline-gray-200 transition-[opacity,transform,width,height,scale,translate] duration-[var(--duration)] ease-[var(--easing)] data-[ending-style]:scale-90 data-[ending-style]:opacity-0 data-[ending-style]:duration-150 data-[starting-style]:scale-90 data-[starting-style]:opacity-0 min-[500px]:w-[var(--popup-width)] xs:w-[var(--popup-width)] dark:shadow-none dark:-outline-offset-1 dark:outline-gray-300">
            <NavigationMenu.Arrow class="flex transition-[left] duration-[var(--duration)] ease-[var(--easing)] data-[side=bottom]:top-[-8px] data-[side=left]:right-[-13px] data-[side=left]:rotate-90 data-[side=right]:left-[-13px] data-[side=right]:-rotate-90 data-[side=top]:bottom-[-8px] data-[side=top]:rotate-180">
              <ArrowSvg />

...
describe('<Select.Root />', () => {
      ...
      render(() => (
        ...
        >
          ...
          <Select.Portal>
            <Select.Positioner>
              <Select.Popup>
                <For each={items}>{(it) => <Select.Item value={it}>{it.country}</Select.Item>}</For>
              </Select.Popup>
            </Select.Positioner>
          </Select.Portal>
        </Select.Root>
  ...
  describe('dynamic items', () => {
    ...
    it('unselects the selected item if removed', async () => {
      function DynamicMenu() {
        ...
        const [selectedItem, setSelectedItem] = createSignal<string | null>('a');
          ...
          <div>
            <Select.Root defaultValue="b">
              ...
              <Select.Portal>
                <Select.Positioner>
                  <Select.Popup>
                    <For each={items()}>{(it) => <Select.Item value={it}>{it}</Select.Item>}</For>
                  </Select.Popup>
                </Select.Positioner>
              </Select.Portal>
            </Select.Root>
    ...
    it('resets via onValueChange and does not break in controlled mode when the selected item is removed', async () => {
      ...
      const { user } = render(() => <TestControlled />);
  ...
  describe('typeahead', () => {
    ...
    it('does not jump matches after a closed-state value reset', async () => {
      function App() {
        const [value, setValue] = createSignal<string | null>('dog');
        return (
          <>
        ...
        >
          ...
          <Select.Portal>
            <Select.Positioner>
              <Select.Popup>
                <For each={users}>
                  {(user) => <Select.Item value={user}>{user.name}</Select.Item>}
                </For>
              </Select.Popup>
            </Select.Positioner>
          </Select.Portal>

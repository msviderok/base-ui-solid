...
const store = SolidStore<TestState, Record<string, never>, typeof selectors>(
  { count: 0, text: '' },
  undefined,
  selectors,
);
...
expectType<Accessor<number | undefined>, typeof countReactive>(countReactive);

const textReactive = store.useState('text');
expectType<Accessor<string>, typeof textReactive>(textReactive);

const isTextLongerThan7Reactive = store.useState('textLongerThan', 7);
expectType<Accessor<boolean>, typeof isTextLongerThan7Reactive>(isTextLongerThan7Reactive);

const isTextLengthBetween2And8Reactive = store.useState('textLengthBetween', 2, 8);
expectType<Accessor<boolean>, typeof isTextLengthBetween2And8Reactive>(
  isTextLengthBetween2And8Reactive,
);
